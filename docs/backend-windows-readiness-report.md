...
| Pairing, sesión y uso único | **PARTIAL** | Windows crea una sesión anónima (`src/ControlParental.Service/DeviceAuthenticator.cs:196-254`) y luego llama pairing (`src/ControlParental.Service/PairingService.cs:70-105`). El backend crea o busca otro usuario y escribe allí `device_id` (`../parentalControl/supabase/functions/pairing/index.ts:91-119`), no devuelve sesión (`../parentalControl/supabase/functions/pairing/index.ts:162-169`) y Windows no refresca el JWT antes de pedir policy (`src/ControlParental.Service/PairingService.cs:163-200`). **La redención atómica ya cubre el claim y todas las escrituras de pairing**: `redeem_pairing_code_atomic` bloquea la fila, escribe child/device/metadata/policy y cambia a `CONSUMED` al final; cualquier excepción revierte la transacción y deja el código `ACTIVE`. Cobertura Deno en `../parentalControl/supabase/functions/pairing/index_test.ts` (7/7 passing: éxito, replay, concurrencia, expirado, código ausente, fallo downstream con retry y formato). El gap abierto es la sesión/JWT: el backend no devuelve tokens ni fuerza el refresh post-pairing. | No verificada; Anonymous Sign-Ins y hook habilitado son desconocidos. |
...
| `time_requests` y atomicidad de aprobación | **COMPLETE FOUNDATION** | La tabla existe (`../parentalControl/supabase/migrations/001_initial_schema.sql:64-75`). Windows envía `request_id`, `minutes` y omite `device_id`/`minutes_requested` (`src/ControlParental.Service/BackendClient.cs:342-368`); el contrato del cliente debe alinearse a `request_id`+`minutes` o el handler debe extender la deducción. A partir de la migración `../parentalControl/supabase/migrations/012_approve_request_atomic.sql`, `approve-request` llama la RPC `approve_request_atomic(...)` que hace `UPDATE time_requests`+`INSERT grants` en una sola transacción (`../parentalControl/supabase/functions/approve-request/index.ts:116-149`); la constraint `grants_request_id_unique UNIQUE(request_id)` (`../parentalControl/supabase/migrations/012_approve_request_atomic.sql:62-63`) garantiza idempotencia en DB y los retries devuelven `idempotent: true` con el `grant_id` original. Cobertura Deno en `../parentalControl/supabase/functions/approve-request/index_test.ts` (9/9 passing: B.1–B.7 atómicos + A.1.9/A.1.9b auto-DENY). | No verificada. |
...
- [x] Hacer aprobación + grant atómicos mediante RPC/transacción y garantizar un solo grant por request. _(Migración `012_approve_request_atomic.sql` + refactor de `approve-request/index.ts`; cobertura B.1–B.7 + A.1.9/A.1.9b en `supabase/functions/approve-request/index_test.ts`.)_
...
- [ ] Corregir pairing para usar la identidad anónima llamante, completar el refresh de JWT con `device_id` y publicar la sesión resultante. _(La redención de código y las escrituras child/device/metadata/policy ya son atómicas mediante `013_pairing_redemption_atomic.sql`; falta sesión/JWT.)_
...
- [ ] Si falla una escritura downstream antes del commit, la transacción no consume el código y un retry puede reintentarlo. _(Cubierto localmente por la RPC `redeem_pairing_code_atomic` y tests Deno; falta validación en staging.)_
...
- [x] La aprobación cambia la request y crea exactamente un grant de forma atómica. _(Cubierto por la RPC `approve_request_atomic` + `UNIQUE(grants.request_id)`; tests B.1–B.6.)_
...
| Pairing backend | `../parentalControl/supabase/functions/pairing/index.ts`; `../parentalControl/supabase/migrations/013_pairing_redemption_atomic.sql` |
...
| Aprobación | `../parentalControl/supabase/functions/approve-request/index.ts` (refactor a RPC); `../parentalControl/supabase/migrations/012_approve_request_atomic.sql` |
...
| Alcance de pruebas backend | `../parentalControl/supabase/functions/pairing/index_test.ts`; `../parentalControl/supabase/functions/get-devices-for-parent/index_test.ts:3-14`; `../parentalControl/supabase/functions/approve-request/index_test.ts:1-13` |

## Declaración de cambios

La revisión inicial de este informe fue estática. En este seguimiento se modificaron la Edge Function de pairing, sus pruebas, la migración RPC de redención atómica, la aprobación atómica/idempotente y las afirmaciones de este informe relacionadas con esos flujos. WNS, realtime e integridad permanecen fuera de alcance.
